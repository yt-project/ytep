...
other field names.  The name ``electron`` will be reserved for electron fields,
as it is unambiguous and will not be utilized elsewhere.
...
As a point of clarification, the ``electron_density`` currently defined for
