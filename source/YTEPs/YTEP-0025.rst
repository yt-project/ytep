...
local machine.  Slices have not yet been implemented.

Profiles
%%%%%%%%

1, 2, and 3D profiles are like 1, 2, and 3D uniform grid datasets where dx, dy, 
and dz are different and have different dimensions.  This may make geometric 
selection difficult, at least for now.  The minimum goal for profile datasets 
is to be able to feed them into ``ProfilePlot`` and ``PhasePlot``.
...
Plot objects could be made to export specialized files allowing them to be 
recreated.  I don't know of any alternatives for exporting geometric 
data containers.
